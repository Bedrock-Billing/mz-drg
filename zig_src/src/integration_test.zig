...
fn writeU32(f: std.fs.File, v: u32) !void {
    var b: [4]u8 = undefined;
    std.mem.writeInt(u32, &b, v, .little);
    try f.writeAll(&b);
}

fn writeI32(f: std.fs.File, v: i32) !void {
    var b: [4]u8 = undefined;
    std.mem.writeInt(i32, &b, v, .little);
    try f.writeAll(&b);
}

fn writeU8(f: std.fs.File, v: u8) !void {
...
test "Full Grouper Chain Integration" {
    ...
    {
        const f = try std.fs.cwd().createFile(cluster_info_path, .{});
        defer f.close();
        // Header
        ...
        // Strings
        try f.seekTo(100);
        try f.writeAll("CODE1");
    }
    defer std.fs.cwd().deleteFile(cluster_info_path) catch {};

    // 1.2 Cluster Map
    const cluster_map_path = "test_int_cluster_map.bin";
    {
        const f = try std.fs.cwd().createFile(cluster_map_path, .{});
        defer f.close();
        // Header
        ...
        try f.writeAll(&cluster_idx);
    }
    defer std.fs.cwd().deleteFile(cluster_map_path) catch {};

    // 1.3 Procedure Attributes (Empty)
    const pr_attr_path = "test_int_pr_attr.bin";
    {
        const f = try std.fs.cwd().createFile(pr_attr_path, .{});
        defer f.close();
    ...
    defer std.fs.cwd().deleteFile(pr_attr_path) catch {};

    // 1.4 PR Patterns (Empty)
    const pr_patterns_path = "test_int_pr_patterns.bin";
    {
        const f = try std.fs.cwd().createFile(pr_patterns_path, .{});
        defer f.close();
    ...
    defer std.fs.cwd().deleteFile(pr_patterns_path) catch {};

    // 1.5 Diagnosis Data
    const dx_data_path = "test_int_dx_data.bin";
    {
        const f = try std.fs.cwd().createFile(dx_data_path, .{});
        defer f.close();
        // Header
        ...
        try f.writeAll("MCC\x00"); // Severity "MCC"
    ...
    defer std.fs.cwd().deleteFile(dx_data_path) catch {};

    // 1.6 DX Patterns
    const dx_patterns_path = "test_int_dx_patterns.bin";
    {
        const f = try std.fs.cwd().createFile(dx_patterns_path, .{});
        defer f.close();
        // Header
    ...
    defer std.fs.cwd().deleteFile(dx_patterns_path) catch {};

    // 1.7 Exclusion IDs (Empty)
    const ex_ids_path = "test_int_ex_ids.bin";
    {
        const f = try std.fs.cwd().createFile(ex_ids_path, .{});
        defer f.close();
        try writeU32(f, 0x434F4445); // Magic
        try writeU32(f, 0); // Num entries
        try writeU32(f, 16); // Entries offset
    ...
    defer std.fs.cwd().deleteFile(ex_ids_path) catch {};

    // 1.8 Exclusion Groups (Empty)
    const ex_groups_path = "test_int_ex_groups.bin";
    {
        const f = try std.fs.cwd().createFile(ex_groups_path, .{});
        defer f.close();
    ...
    defer std.fs.cwd().deleteFile(ex_groups_path) catch {};

    // 1.9 Description Data
    const desc_path = "test_int_desc.bin";
    {
        const f = try std.fs.cwd().createFile(desc_path, .{});
        defer f.close();
    ...
    defer std.fs.cwd().deleteFile(desc_path) catch {};

    // 1.10 Gender MDC (Empty)
    const gender_path = "test_int_gender.bin";
    {
        const f = try std.fs.cwd().createFile(gender_path, .{});
        defer f.close();
        try writeU32(f, 0x47454E44); // Magic
        try writeU32(f, 0); // Num entries
        try writeU32(f, 16); // Entries offset
    }
    defer std.fs.cwd().deleteFile(gender_path) catch {};

    // 1.11 HAC Descriptions (Empty)
    const hac_desc_path = "test_int_hac_desc.bin";
    {
        const f = try std.fs.cwd().createFile(hac_desc_path, .{});
        defer f.close();
    ...
    defer std.fs.cwd().deleteFile(hac_desc_path) catch {};

    // 1.11b HAC Formula (Empty)
    const hac_formula_path = "test_int_hac_formula.bin";
    {
        const f = try std.fs.cwd().createFile(hac_formula_path, .{});
        defer f.close();
    ...
    defer std.fs.cwd().deleteFile(hac_formula_path) catch {};

    // 1.12 Formula Data
    const formula_path = "test_int_formula.bin";
    {
        const f = try std.fs.cwd().createFile(formula_path, .{});
        defer f.close();
        // Header
        ...
        try f.writeAll(&[_]u8{0} ** 8); // Surgical
    ...
    defer std.fs.cwd().deleteFile(formula_path) catch {};
